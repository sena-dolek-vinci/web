...
router.patch("/:id", (req, res) => {
  ...
  const body: unknown = req.body; /*« Je récupère le contenu de la requête 
  (req.body) et je le mets dans body en disant à 
  TypeScript que je ne connais pas encore son type, pour pouvoir le 
  valider ensuite. »
  Express ne sait pas quel type de données tu vas recevoir dans req.body.
  Et comme TypeScript n’a pas d’indication précise, il lui attribue 
  automatiquement le type any.*/
  ...
  /*“Je prends les propriétés title, image, volume et price de l’objet body 
  t je les mets dans des variables séparées portant le même nom.”
  Partual<NewDrink> “Les variables que je déstructure (title, image, etc.) 
  ont la forme d’un objet partiel de type NewDrink.”
  */
